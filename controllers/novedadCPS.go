			...
			if contrato[0].Id != 0 {
				...
				if contratoNuevo.FechaFin.Before(otro_si.FechaFin) {
					...
					if contratoNuevo.FechaInicio.Before(otro_si.FechaFin) {
						...
						fmt.Println("FECHA INICIO MAYOR A FECHA FIN", err)
						c.Data["mesaage"] = "FECHA INICIO MAYOR A FECHA FIN " + err.Error()
						c.Abort("403")
					}
				} else {
...
// @Description Maneja la novedad contractual de Suspensión de Contrato para uno o varios contratos
...
func (c *NovedadCPSController) SuspenderContrato() {
	var suspensiones []models.Suspension
	var contratosProcesados []models.Contrato

	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &suspensiones); err != nil {
		var suspension models.Suspension
		if err := json.Unmarshal(c.Ctx.Input.RequestBody, &suspension); err != nil {
			fmt.Println("Error al unmarshal de los datos de suspensión: ", err)
			c.Data["mesaage"] = "Error service POST: The request contains an incorrect data type or an invalid parameter"
			c.Abort("400")
		}
		suspensiones = append(suspensiones, suspension)
	}

	if len(suspensiones) == 0 {
		c.Data["mesaage"] = "Debe enviar al menos una suspensión"
		c.Abort("400")
	}

	for i := 0; i < len(suspensiones); i++ {
		contratoProcesado, err := suspenderContratoCPS(suspensiones[i])
		if err != nil {
			fmt.Println("Error al suspender contrato: ", err)
			c.Data["mesaage"] = "Error al suspender contrato: " + err.Error()
			c.Abort("400")
		}
		contratosProcesados = append(contratosProcesados, contratoProcesado)
	}

	c.Ctx.Output.SetStatus(201)
	c.Data["json"] = map[string]interface{}{"Success": true, "Status": "201", "Message": "Suspensiones procesadas", "Data": contratosProcesados}
	c.ServeJSON()
}

func suspenderContratoCPS(suspension models.Suspension) (models.Contrato, error) {
	var aux map[string]interface{}
	var contrato []models.Contrato
	...
	query := ""
	if suspension.Id > 0 {
		query = "Id:" + strconv.Itoa(suspension.Id)
	} else {
		query = "NumeroContrato:" + suspension.NumeroContrato + ",Vigencia:" + strconv.Itoa(suspension.Vigencia) + ",Documento:" + suspension.Documento
	}

	if err := request.GetJson(beego.AppConfig.String("UrlTitanCrud")+"/contrato?limit=-1&query="+query, &aux); err != nil {
		fmt.Println("Error al obtener el contrato: ", err)
		return models.Contrato{}, fmt.Errorf("error al obtener el contrato: %s", err.Error())
	}

	LimpiezaRespuestaRefactor(aux, &contrato)
	if len(contrato) == 0 || contrato[0].Id == 0 {
		return models.Contrato{}, fmt.Errorf("el contrato no existe")
	}
	...
	query = "ContratoId:" + strconv.Itoa(contrato[0].Id)
	if err := request.GetJson(beego.AppConfig.String("UrlTitanCrud")+"/contrato_preliquidacion?limit=-1&query="+query, &aux); err != nil {
		fmt.Println("Error al obtener contrato_preliquidacion")
		return models.Contrato{}, fmt.Errorf("error al obtener el contrato preliquidacion: %s", err.Error())
	}

	LimpiezaRespuestaRefactor(aux, &contratoPreliquidacion)
	for i := 0; i < len(contratoPreliquidacion); i++ {
		if err := request.GetJson(beego.AppConfig.String("UrlTitanCrud")+"/detalle_preliquidacion?limit=-1&query=ContratoPreliquidacionId:"+strconv.Itoa(contratoPreliquidacion[i].Id), &aux); err != nil {
			fmt.Println("Error al obtener detalles: ", err)
			return models.Contrato{}, fmt.Errorf("error al obtener detalles: %s", err.Error())
		}
		...
		for j := 0; j < len(detalles); j++ {
			if err := request.SendJson(beego.AppConfig.String("UrlTitanCrud")+"/detalle_preliquidacion/"+strconv.Itoa(detalles[j].Id), "DELETE", &aux, nil); err != nil {
				fmt.Println("Error al eliminar detalle: ", err)
				return models.Contrato{}, fmt.Errorf("error al eliminar detalle: %s", err.Error())
			}
			fmt.Println("Detalle eliminado con éxito")
		}

		if err := request.SendJson(beego.AppConfig.String("UrlTitanCrud")+"/contrato_preliquidacion/"+strconv.Itoa(contratoPreliquidacion[i].Id), "DELETE", &aux, nil); err != nil {
			fmt.Println("Error al eliminar contrato preliquidacion: ", err)
			return models.Contrato{}, fmt.Errorf("error al eliminar contrato preliquidacion: %s", err.Error())
		}
		fmt.Println("contrato preliquidacion eliminado con éxito")
	}

	if err := request.SendJson(beego.AppConfig.String("UrlTitanCrud")+"/contrato/"+strconv.Itoa(contrato[0].Id), "DELETE", &aux, nil); err != nil {
		fmt.Println("Error al ejecutar contrato/id: ", err)
		return models.Contrato{}, fmt.Errorf("error al ejecutar contrato/%d: %s", contrato[0].Id, err.Error())
	}
	fmt.Println("contrato eliminado con éxito")
