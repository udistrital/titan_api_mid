...
func (c *GestionReportesController) TotalNominaPorProyecto() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		...
		IDNomina := strconv.Itoa(v.Preliquidacion.NominaId.Id)
		...
		if err := request.GetJson("http://"+beego.AppConfig.String("Urlargocrud")+":"+beego.AppConfig.String("Portargocrud")+"/"+beego.AppConfig.String("Nsargocrud")+"/vinculacion_docente?limit=-1&query="+query, &vinculaciones); err == nil {
			...
			for _, pos := range vinculaciones {
				...
				query := "PreliquidacionId.Ano:" + ano + ",PreliquidacionId.Mes:" + mes + ",PreliquidacionId.NominaId:" + IDNomina + ",PersonaId:" + strconv.Itoa(IDProveedor) + ",ConceptoNominaId.NaturalezaConceptoNominaId:1"
				...
				query2 := "PreliquidacionId.Ano:" + ano + ",PreliquidacionId.Mes:" + mes + ",PreliquidacionId.NominaId:" + IDNomina + ",PersonaId:" + strconv.Itoa(IDProveedor) + ",ConceptoNominaId.NaturalezaConceptoNominaId:2"
				if err := request.GetJson(beego.AppConfig.String("UrlCrudTitan")+"/detalle_preliquidacion?limit=-1&query="+query2, &aux); err == nil {
					LimpiezaRespuestaRefactor(aux, &d)
					if d != nil {
...
func (c *GestionReportesController) DesagregadoNominaPorFacultad() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		...
		query := "IdResolucion.IdFacultad:" + facultad
		if err := request.GetJson(beego.AppConfig.String("UrlAdministrativa")+"/vinculacion_docente?limit=-1&query="+query, &vinculaciones); err == nil {
			//fmt.Println("hola soy el total de vinculaciones para ese proyecto", len(vinculaciones))
			for _, pos := range vinculaciones {
				...
				query := "PreliquidacionId.Ano:" + ano + ",PreliquidacionId.Mes:" + mes + ",PreliquidacionId.NominaId:" + IDNomina + ",PersonaId:" + strconv.Itoa(IDProveedor) + ",ConceptoNominaId.NaturalezaConceptoNominaId:1"
				if err := request.GetJson(beego.AppConfig.String("UrlCrudTitan")+"/detalle_preliquidacion?limit=-1&query="+query, &aux); err == nil {
					LimpiezaRespuestaRefactor(aux, &devengos)
					if devengos != nil {
						for _, dato := range devengos {
							aux := models.DetallePreliquidacion{}
							if err := formatdata.FillStruct(dato, &aux); err == nil {
								aux.NombreCompleto, _, aux.Documento, _ = InformacionPersona(v.Preliquidacion.NominaId.TipoNominaId.Nombre, aux.NumeroContrato, aux.VigenciaContrato)
								res = append(res, aux)
							}
						}
					}
...
func (c *GestionReportesController) TotalNominaPorDependencia() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		...
		query := "PreliquidacionId.Ano:" + ano + ",PreliquidacionId.Mes:" + mes + ",PreliquidacionId.NominaId:" + IDNomina
		if err := request.GetJson(beego.AppConfig.String("UrlCrudTitan")+"/detalle_preliquidacion?limit=-1&query="+query, &aux); err == nil {
			LimpiezaRespuestaRefactor(aux, &d)
			if d != nil {
				for _, dato := range d {
					if err := request.GetJsonWSO2(beego.AppConfig.String("UrlArgoWso2")+"/informacion_contrato_contratista/"+dato.NumeroContrato+"/"+strconv.Itoa(dato.VigenciaContrato), &temp); err == nil {
				...
				for _, dato := range arregloTotal {
					if dato.ConceptoNominaId.NaturalezaConceptoNominaId.CodigoAbreviacion == "NCN_001" {
						total = total + dato.ValorCalculado
						if dato.ConceptoNominaId.Id == 11 || d[0].ConceptoNominaId.Id == 10 {
							contDev = contDev + 1
						}
					}
					if dato.ConceptoNominaId.NaturalezaConceptoNominaId.CodigoAbreviacion == "NCN_002" {
...
func (c *GestionReportesController) DesagregadoNominaPorDependencia() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		...
		//fmt.Println("Preliquidacion.Ano:"+ano+",Preliquidacion.Mes:"+mes+",Preliquidacion.Nomina.Id:"+IDNomina, "dependencia "+dependencia)
		query := "PreliquidacionId.Ano:" + ano + ",PreliquidacionId.Mes:" + mes + ",PreliquidacionId.NominaIdId:" + IDNomina
		if err := request.GetJson(beego.AppConfig.String("UrlCrudTitan")+"/detalle_preliquidacion?limit=-1&query="+query, &aux); err == nil {
			LimpiezaRespuestaRefactor(aux, &d)
			if d != nil {
				for _, dato := range d {
					if err := request.GetJsonWSO2(beego.AppConfig.String("UrlArgoWsso2")+"/informacion_contrato_contratista/"+dato.NumeroContrato+"/"+strconv.Itoa(dato.VigenciaContrato), &temp); err == nil {
						jsonDocentes, errorJSON := json.Marshal(temp)
						if errorJSON == nil {
							json.Unmarshal(jsonDocentes, &tempDocentes)
							if tempDocentes.InformacionContratista.Dependencia.IdDependencia == dependencia {
		...
		if err := request.GetJson(beego.AppConfig.String("UrlTitanCrud")+"/detalle_preliquidacion?limit=-1&query="+query, &aux); err == nil {
			...
			if len(d) != 0 {
				for _, dato := range d {
					...
					if err := request.GetJsonWSO2(beego.AppConfig.String("UrlArgoWso2")+"/contrato/"+dato.NumeroContrato+"/"+strconv.Itoa(dato.VigenciaContrato), &temp); err == nil {
						...
						if contrato != nil {
							...
							if IDOrdenador == ordenador {
								NombreCompleto, Documento, _ := InformacionPersonaProveedor(dato.PersonaId)
