...
func (c *GestionReportesController) TotalNominaPorProyecto() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		...
		IDNomina := strconv.Itoa(v.Preliquidacion.NominaId)
					...
					if devengos != nil {
						...
						for _, dato := range devengos {
							...
							if err := formatdata.FillStruct(dato, &aux); err == nil {
								var nombreNomina string

								if v.Preliquidacion.NominaId == 412 {
									nombreNomina = "FP"
								} else if v.Preliquidacion.NominaId == 413 {
									nombreNomina = "DP"
								} else if v.Preliquidacion.NominaId == 414 {
									nombreNomina = "CT"
								} else if v.Preliquidacion.NominaId == 415 {
									nombreNomina = "HCH"
								} else {
									nombreNomina = "HCS"
								}
								aux.NombreCompleto, _, aux.Documento, _ = InformacionPersona(nombreNomina, aux.NumeroContrato, aux.VigenciaContrato)
								res = append(res, aux)
							}
						}
					}
		...
		if err := request.GetJson("http://"+beego.AppConfig.String("Urlcrud")+":"+beego.AppConfig.String("Portcrud")+"/"+beego.AppConfig.String("Nscrud")+"/detalle_preliquidacion?limit=-1&query="+query, &d); err == nil {
			if d != nil {
				...
				for _, dato := range arregloTotal {
					...
					if dato.ConceptoNominaId.NaturalezaConceptoNominaId == 424 {
